...
DEFAULT_COLOURS = {
    ...
    b"0": np.array([0, 0, 0],
                   dtype=np.uint8),  # Black background beyond map walls
    ...
    b"P": np.array([159, 67, 255],
                   dtype=np.uint8),  # Generic agent (any player)
...
class MapEnv():
  ...
  def reset(self):
    ...
    for agent in self.agents.values():
      ...
      if self.return_agent_actions:
        ...
        prev_actions = np.array([4 for _ in range(self.num_agents - 1)
                                ]).astype(np.uint8)
        visible_agents = self.find_visible_agents(agent.agent_id)
        observations[agent.agent_id] = {
            "curr_obs": rgb_arr,
  ...
  def color_view(self, agent):
    row, col = agent.pos[0], agent.pos[1]
    view_slice = self.world_map_color[row + self.map_padding -
                                      self.view_len:row + self.map_padding +
                                      self.view_len + 1, col +
                                      self.map_padding - self.view_len:col +
                                      self.map_padding + self.view_len + 1,]
  ...
  def map_to_colors(self, mmap, color_map, rgb_arr, orientation="UP"):
    ...
    if orientation == "UP":
      ...
      for row_elem in range(x_len):
        for col_elem in range(y_len):
          rgb_arr[row_elem, col_elem, :] = color_map[mmap[col_elem,
                                                          x_len - 1 - row_elem]]
    elif orientation == "DOWN":
      for row_elem in range(x_len):
        for col_elem in range(y_len):
          rgb_arr[row_elem, col_elem, :] = color_map[mmap[x_len - 1 - row_elem,
                                                          y_len - 1 - col_elem]]
    elif orientation == "RIGHT":
      for row_elem in range(x_len):
        for col_elem in range(y_len):
          rgb_arr[row_elem, col_elem, :] = color_map[mmap[y_len - 1 - col_elem,
                                                          row_elem]]
    else:
      raise ValueError(f"Orientation {orientation} is not valid")
  ...
  def update_moves(self, agent_actions):
    ...
    for agent_id, action in agent_actions.items():
      ...
      if "MOVE" in action or "STAY" in action:
        ...
        rot_action = self.rotate_action(selected_action,
          ...
          if count > 1:
            ...
            for agent_id in all_agents_id:
              ...
              if move.tolist() in self.agent_pos:
                ...
                curr_conflict_pos = self.agents[
                    conflicting_agent_id].pos.tolist()
            ...
            if conflict_cell_free:
              ...
              agent_by_pos = {
                  tuple(agent.pos): agent.agent_id
                  for agent in self.agents.values()
              }
            # ------------------------------------
        ...
        for agent_id, move in moves_copy.items():
          ...
          if move in self.agent_pos:
            ...
            # It is trying to move into you and you are moving into it
              ...
              if (agent_moves[conflicting_agent_id] == curr_pos and
                  move == self.agents[conflicting_agent_id].pos.tolist()):
  ...
  def single_update_map(self, row, col, char):
    ...
    self.world_map_color[row + self.map_padding,
                         col + self.map_padding] = self.color_map[char]
  ...
  def update_map_fire(
    ...
    """From a firing position, fire a beam that may clean or hit agents
        ...
        """
    agent_by_pos = {
        tuple(agent.pos): agent_id for agent_id, agent in self.agents.items()
    }
    ...
    for pos in firing_pos:
      ...
      for i in range(fire_len):
        if (self.test_if_in_bounds(next_cell) and
            self.world_map[next_cell[0], next_cell[1]] != b"@"):
  ...
  def test_if_in_bounds(self, pos):
    ...
    return 0 <= pos[0] < self.world_map.shape[0] and 0 <= pos[
        1] < self.world_map.shape[1]
  ...
  def find_visible_agents(self, agent_id):
    ...
    return np.array(
        [
            1 if (lower_lim <= agent_tup[0] <= upper_lim and
                  left_lim <= agent_tup[1] <= right_lim) else 0

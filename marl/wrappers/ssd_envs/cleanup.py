...
from marl.wrappers.ssd_envs.maps import CLEANUP_MAP
...
CLEANUP_COLORS = {
    ...
    b"S": np.array([113, 75, 24],
                   dtype=np.uint8),  # Light grey-blue stream cell
...
class CleanupEnv(MapEnv):
  ...
  def __init__(
    ...
    self.potential_waste_area = counts_dict.get(b"H", 0) + counts_dict.get(
        b"R", 0)
  ...
  def compute_probabilities(self):
    ...
    if self.potential_waste_area > 0:
      waste_density = 1 - self.compute_permitted_area(

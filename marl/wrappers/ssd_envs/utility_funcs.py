...
def make_video_from_image_dir(vid_path,
                              img_folder,
...
def make_video_from_rgb_imgs(rgb_arrs,
                             vid_path,
                             video_name="trajectory",
                             fps=5,
                             format="mp4v",
...
def return_view(grid, pos, row_size, col_size):
  ...
  top_edge = y - row_size
  bot_edge = y + row_size
  pad_mat, left_pad, top_pad = pad_if_needed(left_edge, right_edge, top_edge,
                                             bot_edge, grid)
  x += left_pad
  y += top_pad
...
def get_all_subdirs(path):
  return [
      path + "/" + d for d in os.listdir(path) if os.path.isdir(path + "/" + d)
  ]
